...
dds     <- readRDS("R/results/tables/dds_object.rds")
metadata <- read.csv("data/metadata/SraRunTable.csv", stringsAsFactors = FALSE)
...
## 1. VST transformation (for all visualisations) ----
...
pca_plot <- ggplot(pca_data, aes(x = PC1, y = PC2,
  ...
  scale_colour_manual(values = c("Control" = "#2E6DA4",
                                 "FluD4"   = "#E67E22",
                                 "FluD8"   = "#C0392B")) +
  theme_bw(base_size = 13) +
  ggtitle("PCA of VST-normalised counts",
          subtitle = "Mouse lung — Influenza infection time course")

# save plot
ggsave("R/results/figures/PCA_plot.png", pca_plot, width = 7, height = 5, dpi = 300)
cat("PCA plot saved.\n")
...
jpeg("R/results/figures/Sample_distance_heatmap.jpg", width = 480, height = 480, units = "px")
pheatmap(
  samp_mat,
...
comparisons <- list(
  list(file = "R/results/tables/FluD4_vs_Control_all_genes.tsv",
       title = "FluD4 vs Control",
       name  = "FluD4_vs_Control"),
  list(file = "R/results/tables/FluD8_vs_Control_all_genes.tsv",
       title = "FluD8 vs Control",
       name  = "FluD8_vs_Control"),
  list(file = "R/results/tables/FluD8_vs_FluD4_all_genes.tsv",
...
for (comp in comparisons) {
  ...
  ggsave(paste0("R/results/figures/Volcano_", comp$name, ".png"),
...
heat_mat <- vst_mat[all_sig_genes, ]

# Z-score scale each row so colours reflect relative expression
heat_scaled <- t(scale(t(heat_mat)))

jpeg("R/results/figures/Heatmap_sig_DEGs.jpg", width = 480, height = 480, units = "px")
pheatmap(
  heat_scaled,
  annotation_col   = annot_col,
  annotation_colors = annot_colors,
